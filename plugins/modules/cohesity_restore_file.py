...
def get__snapshot_information__for_file(module, self):
    ...
    for filename in self["file_names"]:
        ...
        if "jobRunId" in self:
            output = [
                jobRun
                for jobRun in output
                if jobRun["snapshot"]["jobRunId"] == int(self["jobRunId"])
            ]

        if "backup_timestamp" in self:
            snapshot_timestamp = datetime.strptime(
                self["backup_timestamp"], "%Y-%m-%d:%H:%M"
            ).replace(second=0)
            jobRuns = output
            output = []
            for jobRun in jobRuns:
                t = datetime.strptime(
                    time.ctime(jobRun["snapshot"]["startedTimeUsecs"] / 1000000),
                    "%a %b %d %H:%M:%S %Y",
                ).replace(second=0)
        ...
        if len(output) == 0:
            ...
            module.fail_json(
                ...
                error="No snapshot exists for the file " + filename,
            )
        snapshot_info = output[0]
        restore_details["jobRunId"] = snapshot_info["snapshot"]["jobRunId"]
        restore_details["startedTimeUsecs"] = snapshot_info["snapshot"][
            "startedTimeUsecs"
        ]

        exists = [
            item
            for item in restore_objects
            if item["jobRunId"] == restore_details["jobRunId"]
        ]
        if not exists:
    ...
    if module.check_mode:
        ...
        if job_exists:
            ...
            if environment in ("PhysicalFiles", "GenericNas", "Physical"):
                ...
                if not source_id:
                    module.fail_json(
                        msg="Failed to find the endpoint on the cluster", changed=False
                    )
                ...
                for restore_file in job_details["file_names"]:
                    if environment in ("PhysicalFiles", "Physical"):
                        ...
                        restore_file_list.append(
                            strip__prefix(job_details["endpoint"], restore_file)
                        )
                    else:
