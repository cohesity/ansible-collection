...
- name: Sync objects status in the VM migration task
  cohesity_migration_status:
    cluster: cohesity.lab
    username: admin
    password: password
    state: present
    task_name: "Ansible Migrate VM- Check In Sync"
...
def check__protection_restore__exists(module, self):
    ...
    if module.params.get("task_id"):
        ...
        for task in restore_tasks:
            #module.exit_json(msg=[type(task_id), type(task["id"])])
...
def get_migration_status(module, self):
    """
    Sync Objects available in the migrate job.
    ...
    try:
        uri = "https://" + server + "/irisservices/api/v1/restoretasks/%s" % self["task_id"]
        headers = {
        ...
        if not task_status:
            results = dict(
                ...
                msg="Couldn't find the Restore Job '%s'" % (
                    task_details["name"] or module.params.get("task_id")),
            )
        elif task_status != "kInProgress":
            results = dict(
                changed=False,
                msg="The Restore Job for is %s" % task_status,
                id=task_status,
                name=task_details["name"],
            )
        else:
            task_details["task_id"] = task_id
            response = get_migration_status(
                module, task_details
            )

            results = defaultdict(int)
            if response:
                objects = response[0]["restoreTask"]["restoreSubTaskWrapperProtoVec"]
                for obj in objects:
                    status = obj["performRestoreTaskState"]["base"]["publicStatus"]
                    results[status] += 1
                if len(results.keys()) > 1:
                    msg = "Migration is incomplete, One or more VM is still running."
                    for key, val in results.items():
                        msg += key + '-' + str(val)
                    module.exit_json(msg=msg, changed=False)
                else:
                    module.exit_json(
                        msg="Status of all VM(s) available in the migration task is '%s'" % list(results.keys())[0])
