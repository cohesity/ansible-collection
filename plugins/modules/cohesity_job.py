...
options:
  ...
  disable_indexing:
    default: false
    description: "Enabling this will disable indexing while creating/updating the backup jobs."
    type: bool
  environment:
    choices:
      - VMware
    ...
    if (
        ...
        if job_details["environment"] == "PhysicalFiles":
            msg = (
                ...
                + indexing_msg
                + "sources and filepaths to existing protection job"
            )
        else:
            msg = (
                "Successfully updated "
...
def main():
    ...
    if module.check_mode and module.params.get("state") == "absent":
        ...
        if job_exists:
            ...
            # from the job.
            if (
                not module.params.get("protection_sources")
                or len(module.params.get("protection_sources")) == 0
            ):
                job_details["id"] = job_exists
                ...
                job_details["environment"] = module.params.get("environment")
                existing_job_details = get_prot_job_details(job_details, module)
                sources_exiting_in_job = set(job_details["sourceIds"]).intersection(
                    existing_job_details["sourceIds"]
                )
                if len(sources_exiting_in_job) != 0 and len(
                    sources_exiting_in_job
                ) != len(existing_job_details["sourceIds"]):
                    existing_job_details["sourceIds"] = list(
                        set(existing_job_details["sourceIds"]).difference(
                ...
                elif len(sources_exiting_in_job) != 0 and len(
                    sources_exiting_in_job
                ) == len(existing_job_details["sourceIds"]):
                    module.fail_json(
